...
public class CallingFilter implements IFilter{
    public boolean doTest(Map<Long, IInstruction> instList, int index)
    {
        ...
        if( ins.getInstructiontype() == InstructionType.CALLQ 
                ...
                && ins.getOperands().get(1).getOperandValue() == OperandType.MEMORY_PHYSICAL_ADDRESS )
                ...
                && ins.getOperands().get(1).getOperandValue() == OperandType.REGISTER )
        {
            ...
            if( ins.getInstructiontype() == InstructionType.CALLQ
                    && ins.getOperands().get(0).getOperandType() == OperandType.MEMORY_PHYSICAL_ADDRESS
                    && ins.getOperands().get(1).getOperandValue() == OperandType.MEMORY_PHYSICAL_ADDRESS ) //SectionName
            {
                return true;
            }
        }
