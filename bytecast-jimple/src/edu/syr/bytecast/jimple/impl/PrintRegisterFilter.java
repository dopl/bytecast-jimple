 ...
 * @author Fei Qi
 */
public class PrintRegisterFilter extends AbstractFilter implements IFilter{
    public boolean doTest(List<IInstruction> instList, int index)
    {
        ...
        if(ins.getInstructiontype() == InstructionType.MOV
                && ins.getOperands().get(0).getOperandType() == OperandType.REGISTER // AMD64 group will add this name
                && ins.getOperands().get(0).getOperandValue().toString() != "%rax"
                && ins.getOperands().get(1).getOperandType() == OperandType.REGISTER
                && ins.getOperands().get(1).getOperandValue().toString() == "%rsi")
        {
